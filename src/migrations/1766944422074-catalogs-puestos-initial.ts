...
/**
 * Migración: Crear índices iniciales para la colección puestos
 * Fecha: 2025-01-28
 * 
 * Esta migración crea todos los índices necesarios para la colección puestos:
 * - Índice único: nombre
...
export async function up(connection: Connection): Promise<void> {
  ...
  try {
    ...
    console.log('✓ Índice de texto creado: puesto_text_search');
  } catch (error: any) {
    if (error.code === 85) {
  ...
  console.log('✅ Migración catalogs-puestos-initial completada exitosamente');
}
...
export async function down(connection: Connection): Promise<void> {
  ...
  console.log('Revirtiendo migración catalogs-puestos-initial...');
  ...
  console.log('✅ Reversión de migración catalogs-puestos-initial completada');
}
